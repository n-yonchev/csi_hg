...
#------------------------------------------------------------------------------------------------
# източник : cash2.inc.php - Бъзински 
$arc2type= array();
...
# заявка свободен ПКО 
$quprih= "select 3 as typesuma, prih.id as idsuma
...
# заявка РКО 
$qurazh= "select 9 as typesuma, razh.id as idsuma
...
# сумарна 
$c2qusuma= "
	select sum(if(t2.typesuma=9,0,t2.suma)) as suma1, sum(if(t2.typesuma=9,t2.suma,0)) as suma2
	from (
		($qufina) 
			union all 
		($quadva)
