...
# постъпили суми през месец 
# - датата на постъпление се определя чрез полето finance.dateinco 
...
if (empty($year) or empty($mont)){
						...
						<form method=post>
								...
								<tr>
						<td> описание
						<td> постъпили (събрани) суми през месец 
								<tr>
						<td> година
...
*/
	...
	$yemo= "$year-$mont";
	$arhead[]= "суми, постъпили през месец $mont-$year";
	$cdatfilt= "substr(finance.dateinco,1,7)='$yemo'";
$txhead= implode("<br>",$arhead);
...
$userlist= $DB->selectCol("select id as ARRAY_KEY,name from user order by name");
	...
	$query= "
		...
		where $cdatfilt and suit.iduser=?d
