{*----
		26.07.2010 - специфичен шаблон за наблщдател 
		клонинг на case3.tpl-case.tpl 
		отпадат колоните : 
			последна промяна 
			входящи документи 
		допълнит.колони : 
...
.membtype {ldelim}background-color:#666666;color:white;padding:1px 4px 1px 4px;{rdelim}
</style>
{assign var=tdst value="style='border-bottom: 1px solid #666666'"}
{*
...
<form name="myseleform" method=post style="margin:0px;padding:0px;width:auto;" enctype="multipart/form-data">
		<table class="d_table" width='' cellspacing='0' cellpadding='0' align=center>
			<thead>	
				<tr>
	{if $VIEWUSERNAME}
				<td class='d_table_title' colspan='10'> списък на наблюдаваните дела
{*----
...
>
<td {$tdst}> {$elem.serial}/{$elem.year}</td>
				<td class='sep'>&nbsp;</td>
{*
				<td> {$elem.text}</td>
*}
<td {$tdst} align=center>
		{if empty($elem.text)}
&nbsp;
		{else}
<img src="images/view.png" title="{$elem.text}">
		{/if}
				{assign var="arindx" value=$elem.idcofrom} 
				<td class='sep'>&nbsp;</td>
<td {$tdst}> {$ARFROM.$arindx}</td>
				<td class='sep'>&nbsp;</td>
<td {$tdst}> {$elem.created|date_format:"%d.%m.%Y"}</td>
{*
				<td class='sep'>&nbsp;</td>
...
------*}
				<td class='sep'>&nbsp;</td>
<td {$tdst}> {$elem.username} &nbsp;</td>
				<td class='sep'>&nbsp;</td>
<td {$tdst}> {include file="case4list.tpl" LIST=$elem.listclai}</td>
				<td class='sep'>&nbsp;</td>
<td {$tdst}> {include file="case4list.tpl" LIST=$elem.listdebt}</td>
{*--------*}
{*
				<td class='sep'>&nbsp;</td>
<td {$tdst} align=center>
...
{*
				<td class='sep'>&nbsp;</td>
<td {$tdst} align=center onclick="event.cancelBubble=true;document.location.href='{$elem.lockmy}';">
...
*}
<td {$tdst} align=center>
		{assign var='txstat' value=$ARSTAT[$elem.idstat]}
		{if empty($txstat)}
&nbsp;
		{else}
<img src="images/view2.gif" title="{$txstat}">
		{/if}
				{*---- статуса за основ.данни - брояч ----*}
{*
				{if $ISBASESTATUS and !$VIEWUSERNAME}
...
function grouparch(){ldelim}
			var lire= "";
{*
	$(":input").each(function(){ldelim}
//alert(this.name+'/'+this.type+'/'+this.value+'/'+this.checked);
				if (this.type=="checkbox"){ldelim}
//alert(this.checked);
					lire += (this.checked) ? ","+this.value : "";
				{rdelim}else{ldelim}
				{rdelim}
	{rdelim});
*}
