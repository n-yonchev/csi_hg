...
.addr {ldelim}font:normal 7pt verdana;color:blue;{rdelim}
				...
				{if $CALLUSER}
<style>
.curr2 {ldelim}cursor:pointer;font:normal 8pt verdana;padding:1px 6px;border-bottom: 1px solid darkgreen;color:darkgreen;background-color:lightgreen;{rdelim}
.vari2 {ldelim}cursor:pointer;font:normal 8pt verdana;padding:1px 6px;border-bottom: 1px solid darkgreen;color:darkgreen;{rdelim}
</style>
				{else}
				{/if}

				{if isset($NAMELOCKED)}
<span class="no">{$NAMELOCKED} е в това дело</span>
				{else}
				{/if}
									<table align=center>
												{if $CALLFROMALTE}
...
<a class="{if $CASEALTE==-9}curr2{else}link{/if}" {include file="_href.tpl" LINK=$GOBACK}>
постъпления готови за превод [стр.{$PAGE}] </a>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<span class="seye">
...
ръчни </a>
			...
			{if isset($smarty.session.lastcase)}
<span class="seye">
последни дела 
</span>
&nbsp;
				{foreach from=$smarty.session.lastcase item=elem key=idcase}
<a class="{if $idcase==$CURRCASE}curr2{else}link{/if}" {include file="_href.tpl" LINK=$elem.caselink}> {$elem.caseseri}/{$elem.caseyear} </a>
&nbsp;
				{/foreach}
			{else}
			{/if}
...
{*
												{if $CALLFROMALTE}
*}
												{if $CALLFROMALTE and !$CALLUSER}
												{else}
									<tr>
									<td>
{*---- подменю ----*}
				{foreach from=$ARSUBM item=elemvari key=ekeyvari}
							{if $ARCOUN.$ekeyvari==0}
							{else}
...
*}
<td align=center colspan={if $isiban}3{else}2{/if}> разпределение
												{/if}
											{/if}
		<tr class='head2'>
<td align=right> сума
...
<nobr>{$elem.cashdate}</nobr>
						{elseif $idtype==7}
<nobr>{$elem.created|date_format:"%d.%m.%Y"|cat:" въвед"}</nobr>
						{else}
&nbsp;
						{/if}
{***
...
<span class="yes" title="{$oktext}">&nbsp;</span>
									{else}
<img src="images/finish.gif" title="СТАРО ПРИКЛЮЧЕНО на {$elem.timeclosed|date_format:'%d.%m.%Y %H:%M:%S'}"
oncontextmenu="document.location.href='{$elem.demarkclos}';return false;">
									{/if}
					{elseif $elem.idfinastat==6}
								{if $CALLUSER}
...
{*---- продължение на описанието ----*}
								...
								{if $first}
<td {$myspan} align=center>
					{if $elem.idfinastat==1 and empty($elem.lockname)    and !$CALLUSER}
<img src="images/unmark.gif" style="cursor:pointer;" {include file="_href.tpl" LINK=$elem.finaback} title="върни на деловодителя за корекция"> 
					{else}
&nbsp;
					{/if}
<td {$myspan} align=center>
...
*}
						{if $elemrefe.idstat==9}
<td bgcolor={$ARPACKCOLO[2]}> превед
						{elseif $elemrefe.idstat==6}
