...
{foreach from=$LIST item=elem key=ekey}
	<tr onmouseover='this.className="tr_hover";' onmouseout='this.className="";' >
		<td> {$elem.name}</td>
		<td class='sep'>&nbsp;</td>
<td> {if $elem.idtype==0}съд{else}<nobr>друг изт.</nobr>{/if}</td>
		<td class='sep'>&nbsp;</td>
...
<td align=center><a href="{$elem.view}" class="nyroModal" target="_blank"><img src="images/view.png" title="подробно"></a></td>
		<td class='sep'>&nbsp;</td>
	{if isset($smarty.session.getcofrom)}
<td align=center><a href="{$elem.put}"><img src="images/put.gif" title="спусни тук {$smarty.session.getcofromname}"></a></td>
	{else}
