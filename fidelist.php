<?php
# извежда списък дела по критерий за длъжник 
# отгоре : 
#    $iduser - логнатия потребител 
#    $mode - текущия режим 
# и още отгоре : 
#    $seek - стринга за търсене на длъжник 
//print_r($GETPARAM);
//var_dump($seek);
...
# назад към списъка с длъжници 
$smarty->assign("PAGEBACK", $page);
$smarty->assign("PAGEBACKTEXT", "назад към списъка длъжници");
...
//					$elname= sprintf("upper(%s) like upper('%s')"  ,"debtor.name",$text4);
//$elname= sprintf("(upper(%s) like upper('%s') or upper(%s) like upper('%s'))"  ,"debtor.name",$text4,"debtor.name2",$text4);
$elname= sprintf($tempall  ,"debtor.name",$text4,"debtor.name2",$text4);
					# - за идентификатора - едновременно юридич/физич лица 
//						$elegnn= sprintf("upper(%s) like upper('%s')"  ,"debtor.egn",$text4);
# 29.05.2009 Бъзински - търсене в ЕГН, включително и на съпруга 
//$elegnn= sprintf("(upper(%s) like upper('%s') or upper(%s) like upper('%s'))"  ,"debtor.egn",$text4,"debtor.egn2",$text4);
$elegnn= sprintf($tempall  ,"debtor.egn",$text4,"debtor.egn2",$text4);
						$elbuls= sprintf("upper(%s) like upper('%s')"  ,"debtor.bulstat",$text4);
					$eliden= "case idtype when 1 then ($elbuls) when 2 then ($elegnn) when 3 then 0 else 0 end";
# 29.05.2009 Бъзински - търсене в адреса, включително и на съпруга 
$eladdr= sprintf($tempall  ,"debtor.address",$text4,"debtor.address2",$text4);
					# филтъра 
...
//		$query= "select * from debtor where $filter order by name";
		$query= "select debtor.*, debtor.id as id
...
$pagecont= smdisp("fidelist.tpl","fetch");
