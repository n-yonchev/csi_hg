<?php
# събрани пари в-брой по деловодители за дата/период - избора и списъка заедно 
# формата и списъка - заедно 
# източник : 
#    fdda.php - търсене на вх.документ по дата/период 
...
/****
									# извеждане списък събирачи по въведен филтър 
									$view= $GETPARAM["view"];
									if (isset($view)){
										list($d1cano,$d2cano)= explode("^",$view);
											...
											$d2bg= "$da.$mo.$ye";
$fielem= "str_to_date(finance.cashdate,'%d.%m.%Y')";
										if (empty($d2cano)){
# филтъра 
$filter= "$fielem='$d1cano'";
$smarty->assign("FILTTX", "събрани на $d1bg");
										}else{
# филтъра 
$filter= "$fielem>='$d1cano' and $fielem<='$d2cano'";
$smarty->assign("FILTTX", "събрани в периода $d1bg - $d2bg");
										}
										include_once "finacashpers.php";
# ВНИМАНИЕ. 
...
#------ начало 
							...
							$retucode= -1;
	$_POST["date1"]= date("d.m.Y");
							$retucode= 0;
	$date1= $_POST["date1"];
		$d1cano= bgdateto($date1);
		$d2cano= "";
...
//	$pagecont= smdisp("finacash.tpl","fetch");
}

	# извеждаме формата с филтъра 
	$smarty->assign("FILIST", $filist);
//	$pagecont= smdisp("finacash.tpl","fetch");
	$pagefo= smdisp("finacash.tpl","fetch");
	
									$pagecont= $pagefo .$pagecont;


function viewaction($view,$fielem){
global $smarty;
										list($d1cano,$d2cano)= explode("^",$view);
											list($ye,$mo,$da)= explode("-",$d1cano);
											$d1bg= "$da.$mo.$ye";
