...
function spoubegi_sumi($fielname){
global $DB, $smarty;
...
$idcase= $_SESSION["IDCASECREA"];
		...
		//$mylist= $DB->select("select * from subject where idcase=$idcase");
										# 21.07.2010 - Дичев 
										# четем предметите само по изп.лист = всички олихвяеми + неолихв.от тип по изп.лист 
										$filt2= "(idtype=1 or idtype=2 and idsubtype=4)";
...
function createar_sumi($fielname){
								$artextarea= array();
