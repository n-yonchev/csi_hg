...
function getxml($filename, $full)
{
	...
	$arfiel = array(
		"inco" => "\$elem['Amount']['value']"
	, "date" => "substr(\$elem['PaymentDateTime']['value'],0,10)"
	, "hour" => "substr(\$elem['PaymentDateTime']['value'],11)"
	, "refe" => "\$elem['DocumentReference']['value']"
	, "desc1" => "\$elem['NarrativeI02']['value']"
	, "desc2" => "\$elem['ReasonI02']['value']"
	, "desc" => "\$eldata['desc1'].' '.\$eldata['desc2']"
	, "type" => "\$elem['MovementTypeForExport']['value']"
	, "op_name" => "\$elem['OppositeSideName']['value']" // 23.01.2024 добавена нова стойност от xml фалйа - "OppositeSideName" която се запазва в БД в таблица "finasource" в поле "op_name" 
	);
	...
	$armove = $arcont["Items"]["AccountMovement"];
	//print_rr($armove);
	$ardata = array();
	# трансформация
    ...
    */
	...
	foreach ($ardata as $elindx => $eltran) {
		...
		# приход/разход
		if ($eltran["type"] == "2" || $eltran["type"] == "Credit") {
	...
	# заглавните данни
	$arhead = $arcont["Items"]["AccountMovement"][0]["Account"];
	//print_r($arhead);
