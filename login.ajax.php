...
#------ submit без формални грешки 
				...
				$idus= 0;
											...
											$rouser= $DB->selectCol("select id from user where username=? and password=?" ,$username,$mdpass);
				...
				$errtype= 3;
													...
													$idus= $rouser[0];
													$rowork= $DB->selectCol("select inactive from user where id=?d" ,$idus);
									//var_dump($idus);
									//print_r($rowork);
													if ($rowork[0]==0){
													}else{
...
//									$editel= "mode=$mode&page=$page&filt=$filt&edit=$edit";
	...
	print smdisp("login.ajax.tpl","iconv");
}
