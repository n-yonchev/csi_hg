<?php
# групово приключване на изравнени извлечения 
# - вика се самостоятелно във nyroModal прозорец - виж cazofina.tpl 
# източник : finaclos.ajax.php - приключва единично постъпление 
# отгоре : 
#    $para - списък с finance.id за приключване 
# ВМЕСТО $clos - finance.id за приключване 
...
$arpara= explode("/",$para);
$clos= $arpara[0];
$smarty->assign("COUN", count($arpara));
				$suma= 0;
foreach($arpara as $elem){
	$rofina= getrow("finance",$elem);
				$suma += $rofina["inco"];
}
$smarty->assign("SUMA", $suma);

# шаблона 
$tpname= "cazofinaclosgr.ajax.tpl";
# полетата 
//$filist= array();
$filist= array(
...
//$DB->query("update finance set ?a, isclosed=1, jourtime=now() where id=?" ,$aset,$clos);
	...
	finaarchive($clos);
								...
								+++*/
													$DB->query("lock tables finance write, suit write, finahist write, user write");
						$rocase= getrow("suit",$rocont["idcase"]);
						$loby= $rocase["lockedby"];
									# проверваме в последния момент дали делото отново не е отворено 
									#     suit.lockedby=0 - не е отворено 
									#     suit.lockedby=$iduser - отворено е от логнатия 
//print "loby=[$loby][$iduser]";
						if ($loby==0 or $loby==$iduser){
							# ОК - приключваме постъпленията 
	foreach($arpara as $finaid){
		$DB->query("update finance 
			set ?a, isclosed=1, timeclosed=now()
			where finance.id=?d
			"  ,$aset,$finaid);
		finaarchive($finaid);
	}
						}else{
							# делото е отворено преди тази заявка 
							# - извеждаме отново формата с информационно съобщение 
							$rouser= getrow("user",$loby);
	$smarty->assign("LOCKNAME", $rouser["name"]);
							$retucode= -9;
							$smarty->assign("NOEF", true);
						}
