...
# статуси за регистъра 
$statlist= array();
# спряно 
$statlist["stop"]= array(8);
# прекратено вкл.перемирано =4 
//$statlist["term"]= array(123,124,125,126,127,128,129);
$statlist["term"]= array(4,  121,122,123,124,125,126,127,128,129);
# свършено 
//$statlist["fini"]= array(16,121,122);
$statlist["fini"]= array(16);
# възобновено 
$statlist["cont"]= array();
# изпратено 
$statlist["sent"]= array(201,202);
...
$mylist= $DB->select("
	select id, year, serial, date(created) as casedate
, idstat, date(timestat) as statdate
	from suit
	where $cond1
	order by year, serial
	");
$mystat= $DB->select("
	select suitstathist.idstat as idstat, date(suitstathist.time) as statdate
		,suitstathist.idcase as ARRAY_KEY
	from suitstathist
		left join suit on suitstathist.idcase=suit.id
	where $cond2
	order by suitstathist.time desc
	");
...
****/
			...
			#---- висящо или възобновено ----
			$datecont= "";
			if (empty($arwork)){
				$arhist= $mystat[$idcase];
				if (empty($arhist)){
				}else{
					foreach($arhist as $elhist){
						$idstathist= $elhist["idstat"];
						if (in_array($idstathist,$statlist["stop"])){
							$datecont= $statdate;
							break;
						}else{
						}
						if (in_array($idstathist,$statlist["term"])){
							$datecont= $statdate;
							break;
						}else{
						}
						if (in_array($idstathist,$statlist["fini"])){
							$datecont= $statdate;
							break;
						}else{
						}
...
//			$aset[]= $ardata["t1stat"];
			...
			$aset[]= $arname[2];
//			$aset[]= "";
# 14.06.2011 - след кореспонденция с авторите 
$aset[]= $elem["name"];
					if ($ardata["t2et"]==0){
			$aset[]= 1;
					}else{
