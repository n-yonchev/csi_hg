...
# данните 
$myquery= getcertqu() ." where aadocuc2.id=$word";
...
//$artags["REGIOUTNUMB"]= $data[0]["outseri"];
//$artags["REGIOUTDATE"]= bgdatefrom($data[0]["outdate"]);
$artags["REGIOUTNUMB"]= $certdata["queryexitdocnumber"];
$artags["REGIOUTDATE"]= $certdata["queryexitdocdate"];

//$artags["REGIPERS"]= $data[0]["adresat"];
//$artags["REGIIDEN"]= $data[0]["param"];
$artags["REGIPERS"]= $certdata["personname"];
$artags["REGIIDEN"]= $certdata["queryparam"];
...
/*
		$reginu= $DB->selectCell("select count(*) from aadocuc2 where id<?d"  ,$data[0]["id"]);
$artags["REGICERTNUMB"]= $reginu +1;
*/
$artags["REGICERTNUMB"]= $certdata["queryclientid"];
...
//var_dump($trow);
		...
		foreach($arcase as $caelem){
			...
			$mytrow= str_replace("CASEBB",$caelem["execcasefullnumber"]." ".$caelem["currentstatusdesc"],$mytrow);
