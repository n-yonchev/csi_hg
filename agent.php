...
$smarty->assign("FILT", tran1251($filt));
									...
									$edit= $GETPARAM["edit"];
									if (isset($edit)){
										include_once "agentedit.ajax.php";
										exit;
									}else{
									}
									...
									$dele= $GETPARAM["dele"];
									if (isset($dele)){
										include_once "agentdele.ajax.php";
										exit;
									}else{
									}
									...
									if (isset($listcase)){
										include_once "agentlistcase.ajax.php";
										exit;
									}else{
									}
									...
									if (isset($makeeq)){
										include_once "agentmakeeq.ajax.php";
										exit;
									}else{
									}
...
if (empty($filt)){
	...
	$where= "lower(name) like lower('$myfilt')";
		...
		$query= "select * from agent where $where order by name";
		$prefurl= "";
//		$baseurl= "mode=".$mode;
...
$pagecont= smdisp("agent.tpl","fetch");
